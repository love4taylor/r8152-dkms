...
从 GitHub Releases 下载适用于目标发行版的软件包：

| Release 文件                                       | 构建环境            | 软件包格式                     |
| ------------------------------------------------ | --------------- | ------------------------- |
| <code>r8152-dkms_2.22.1-1_all.deb</code>       | Debian 和 Ubuntu | <code>.deb</code>         |
| <code>r8152-dkms-2.22.1-1-any.pkg.tar.zst</code> | 当前 Arch Linux   | <code>.pkg.tar.zst</code> |
...
这些制品是 DKMS 源码包，而不是绑定特定内核版本的预编译模块。请先安装目标
内核对应的头文件；DKMS 会在目标系统上编译并安装 <code>r8152.ko</code>。
...
如果需要为当前运行内核之外的内核构建模块，可以覆盖 <code>KERNELDIR</code>：
...
| <code>debian/</code>                  | Debian 和 Ubuntu 软件包元数据   |
| <code>packaging/arch/PKGBUILD</code>  | Arch Linux 软件包构建脚本       |
...
驱动源码声明 <code>Copyright (c) 2024 Realtek Semiconductor Corp. All rights reserved.</code>，
